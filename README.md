...
复制报告不预设优化结论。默认分析要求覆盖任务推进、执行与交付质量风险、协作委派、失败重试、并发等待、上下文缓存、成本和异常变化，区分事实、推断与待验证项，不重复处理已确认无需改动的配置方向。

- 修复 OMP 子任务外层模板导致的标题和正文哈希匹配错误；用结构化 task 结果的实际输出 ID、运行时父工具记录、规范化正文哈希及明确名称关联委派，保留证据与歧义。
- 父实例与当前实例分开；主控跨任务时列出任务分布。账本按调用、各类 Token、金额分别守恒，导出总计和主体／提供商／模型维度独立复核。
- 数据质量、逐实例采集起止与缺口、Advisor 生命周期提前展示。未记录处置不是 open；上下文观察不再依赖请求钩子，建议正文保留以支持独立分析。
- Markdown 有首尾标记、正文 UTF-8 字节数和 SHA-256；JSON 有可验证完整性元数据。原生剪贴板尽量回读核对，回读不符报错，OSC 52 明确未验证，大报告可以从预览按 `s` 保存并回读核对。

校验保存或转交后的报告：`npm run verify:report -- /path/to/report.md`（也支持 JSON）。校验通过只证明传输正文完整，不代表历史源日志、价格、归因或运行时事件齐全。默认诊断版保留全量汇总，但部分调用／区间证据明确抽样；完整 Markdown 和 JSON 保留全量明细。
...
界面、预览、复制和保存共用同一冻结报告。打开预览不会偷偷重新扫描；复制内容与预览 payload 一致。可选完整 Markdown 或格式 v4 JSON，保留完整调用和事件。紧凑报告仅对高金额调用、重复状态候选和最长活动间隔等明细明确选样，汇总仍基于全量。
...
node scripts/verify-package.mjs dist/omp-session-cost-0.8.0.tgz
```
