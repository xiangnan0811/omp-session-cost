...
const safeId = value => displayName(value, "unknown");
...
export function buildDiagnosticData(report, options = {}) {
  ...
  const agentName = name => displayName(name, "未记录名称");
  ...
  const rows = (items, kind) => (items || []).map(row => ({
    ...(options.includeEvidence ? { reviewedLabel: displayName(row.agent || row.name) } : {}),
  ...
  const calls = sourceCalls.map((c, index) => ({
    ref: ref(c.recordKey || `legacy-${index}`), key: c.recordKey || null, eventId: c.entryId || null,
    ...
    tools: (c.toolFacts || []).map(t => ({ name: t.name, behavior: t.behavior,
      operation: t.operation ?? t.statusOps?.[0]?.op ?? null, target: t.target ?? null, recipient: t.recipient ?? null, broadcast: t.broadcast ?? null,
      statusOps: (t.statusOps || []).map(op => ({ name: op.name, op: op.op, timeoutMs: op.timeoutMs, targets: op.targets || [], partial: op.partial || false, jobNames: op.ids.map(id => displayName(id)), jobRefs: op.ids.map(id => ref(id)) })),
      resultRef: ref(t.result?.eventKey), completeResult: t.result?.complete ?? null, statusComparable: t.result?.statusComparable ?? null, statusKind: t.result?.statusKind ?? null, emptyInbox: t.result?.emptyInbox ?? null,
  ...
  return finalizeEvidence({
    ...
    snapshot: report.snapshot ? { strategy: report.snapshot.strategy, startedAt: iso(report.snapshot.startedAt), frozenAt: iso(report.snapshot.frozenAt),
      ...
      files: report.snapshot.files.map(f => ({ ref: f.fileKey, name: f.localPath || f.name || f.fileKey, sessionFormatVersion: numberOrNull(f.sessionFormatVersion), frozenAt: iso(f.frozenAt), bytes: f.bytes, records: f.records,
    ...
    ledger: report.ledger || null, telemetry: report.telemetry || null, comparison: report.comparison || null,
    observerStatus: report.observerStatus || null,
    unknowns: [
...
export function diagnosticMarkdown(data, { full = false, language = "zh" } = {}) {
  ...
  const lines = ["# OMP Session Cost Analysis Bundle", "", `Generated by: ${data.generatedBy}`, `Generated at: ${data.generatedAt}`,
    ...
    "Rule repeated-status-v2: adjacent complete known status calls with unchanged comparable snapshots (legacy empty inbox, native empty/running wait, or proc snapshot), no delivered message/completion/interruption and no intervening user/custom/configuration notification. Only named snapshot duration/age fields are ignored; logs, output and state changes remain evidence. Dynamic eval and partial/unverified results are excluded; candidates are not proven wasted waits.",
